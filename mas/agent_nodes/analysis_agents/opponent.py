...
class Opponent(BaseAgent):
    ...
    async def process_state(
        ...
        debate_process = self.before_call_opponent(
        ...
        debate_process = self.after_call_opponent(
            opponent_message=debate_result.ai_message,
            decision_shared_messages=debate_process,
        )
        logger.trace(f"\nDebate Process: \n{debate_process}")
        return dict(
    ...
    # ==== 工具方法。 ====
    def after_call_opponent(
        self,
        opponent_message: AIMessage,
        decision_shared_messages: list[AnyMessage],
    ) -> list[AnyMessage]:
        assert isinstance(opponent_message, AIMessage)
        assert len(decision_shared_messages) == 1
        # 标注身份。
        opponent_last_message_content = ContentAnnotator.safe_annotate_with_html(
            tag='opponent',
            original_text=opponent_message.content,
        )
        # 构建 debate_process 。
        ## 不做复杂处理，共享记忆池。
        debate_process_content = decision_shared_messages[0].content + "\n" + opponent_last_message_content
        debate_process = [
            HumanMessage(content=debate_process_content),
        ]
        assert len(debate_process) == 1
