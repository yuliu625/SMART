...
class BaseAnalyst(BaseAgent):
    ...
    def process_state(
        ...
        analyst_result = self.analyze(
            chat_history=state.analyst_chat_history,
            remaining_retrieve_rounds=state.remaining_retrieve_rounds,
        )
        if state.remaining_retrieve_rounds == 0 or analyst_result.agent_request.agent_name == 'validator':
            # 已经完成分析。
            return dict(
                analyst_chat_history=analyst_result.chat_history,
                current_agent_name='validator',
                current_message="",
                analyst_submission=ContentAnnotator.annotate_with_html_comment(
                    tag=self.agent_name,
                    original_text=analyst_result.chat_history[-1].content,
                )
            )
        else:
            # 需要更多信息。
            return dict(
                analyst_chat_history=analyst_result.chat_history,
                current_agent_name='document-reader',
                current_message=analyst_result.agent_request.agent_message,
                remaining_retrieve_rounds=state.remaining_retrieve_rounds-1,
            )
    ...
    def analyze(
        ...
        chat_history = self._before_call_analyst(
    ...
    def _before_call_analyst(
        ...
        if remaining_retrieve_rounds == 0:
            ...
            last_round_content = (
                ...
                + f"\n\n还剩{remaining_retrieve_rounds}次查询次数。"
            )
