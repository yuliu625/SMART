...
# MAS
## Single Agent
from mas.graphs.cached_graphs.cached_single_agent_graph_builder import CachedSingleAgentGraphBuilder
from mas.schemas.single_agent_state import SingleAgentState
## Sequential Workflow
from mas.graphs.cached_graphs.cached_sequential_workflow_graph_builder import CachedSequentialWorkflowGraphBuilder
from mas.schemas.sequential_workflow_state import SequentialWorkflowState
## Multi-Agent Debate
from mas.graphs.cached_graphs.cached_multi_agent_debate_graph_builder import CachedMultiAgentDebateGraphBuilder
from mas.schemas.multi_agent_debate_state import MultiAgentDebateState
## final MAS
from mas.graphs.cached_graphs.cached_final_mas_graph_builder import CachedFinalMASGraphBuilder
...
class CachedGraphFactory:
    @staticmethod
    def create_cached_single_agent_graph(
        ...
        raise NotImplementedError("暂时以独立缓存方法实现。")
        graph_builder = CachedSingleAgentGraphBuilder(state=SingleAgentState)
    ...
    @staticmethod
    def create_cached_sequential_workflow_graph(
        # Surveyor
        ...
        graph_builder = CachedSequentialWorkflowGraphBuilder(state=SequentialWorkflowState)
        # Surveyor
    ...
    @staticmethod
    def create_cached_multi_agent_debate_graph(
        # Surveyor
        surveyor_main_llm: BaseChatModel,
        surveyor_main_llm_system_message: SystemMessage,
        # Proponent
        ...
        graph_builder = CachedMultiAgentDebateGraphBuilder(state=MultiAgentDebateState)
        # Surveyor
        surveyor = AgentFactory.create_surveyor(
            main_llm=surveyor_main_llm,
            main_llm_system_message=surveyor_main_llm_system_message,
    ...
    @staticmethod
    def create_cached_final_mas_graph(
        # Surveyor
        surveyor_main_llm: BaseChatModel,
        surveyor_main_llm_system_message: SystemMessage,
        # Investigator
        ...
        graph_builder = CachedFinalMASGraphBuilder(state=FinalMASState)
        # Surveyor
        surveyor = AgentFactory.create_surveyor(
            main_llm=surveyor_main_llm,
            main_llm_system_message=surveyor_main_llm_system_message,
