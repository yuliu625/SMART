...
from mas.schemas.structured_output_format import InvestigatorRequest
...
def make_test_investigator() -> Investigator:
    investigator = Investigator(
        ...
        main_llm_system_message=cast(
            ...
            PromptTemplateLoader.load_system_message_prompt_template_from_j2(
                system_message_prompt_template_path=r"D:\document\code\paper\SMART\mas\prompts\final_mas\investigator_main_llm_system_prompt_template.j2",
            ).format()
        ),
        ...
        schema_pydantic_base_model=InvestigatorRequest,
        formatter_llm_system_message=cast(
            SystemMessage,
            PromptTemplateLoader.load_system_message_prompt_template_from_j2(
                system_message_prompt_template_path=r"D:\document\code\paper\SMART\mas\prompts\final_mas\investigator_formatter_llm_system_prompt_template.j2",
            ).format()
        ),
    )
    return investigator
...
class TestInvestigator:
    @pytest.mark.parametrize(
        "investigator, state", [
        (make_test_investigator(),
         make_test_final_mas_state()),
    ])
    @pytest.mark.asyncio
    async def test_investigator_as_node_in_final_mas(
        self,
        investigator: Investigator,
        state: FinalMASState,
    ):
        logger.trace(f"\nTest State: \n{state}")
        result = await investigator.process_state(
            state=state,
            config=None,
        )
        logger.info(f"\nSurveyor Result: \n{result}")
